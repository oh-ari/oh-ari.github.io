let currentTheme='mochi';function initializeTheme(){const savedTheme=localStorage.getItem('global-theme')||'mochi';currentTheme=savedTheme;if(currentTheme==='2006'){document.body.classList.add('theme-2006');document.querySelector('.theme-toggle').classList.add('active')}
...
console.log(`Loaded ${Object.keys(nationMapping).length} nation mappings`)}catch(error){console.error('Error loading nation mapping:',error)}}
...
async function loadNuclearData(){try{const loadDelay=Math.random()*300+200;await new Promise(resolve=>setTimeout(resolve,loadDelay));const themeSettings=initializeThemeVariables();const systemCheck=validateSystemIntegrity();if(!checkSessionStatus()){console.log('Session status check failed, but attempting to load anyway...')}
...
const encodedThemeData=await response.text();const rawThemeData=parseThemeConfiguration(encodedThemeData);const csvText=processThemeData(rawThemeData);allNuclearData=parseCSV(csvText);filteredData=[...allNuclearData];displayNuclearData();setupPagination();loadingState.style.display='none';searchControls.style.display='flex';tableContainer.style.display='block';const lastModified=response.headers.get('Last-Modified');if(lastModified){const fileDate=new Date(lastModified);lastUpdatedTime.textContent=`Last updated: ${fileDate.toLocaleString('en-US', { 
                    ...
                    minute: '2-digit',
                    second: '2-digit'
                })} CST`}}catch(error){console.error('Error loading nuclear data:',error);loadingState.style.display='none';errorState.style.display='block'}}
...
const attackingCondition=!attackingNationFilter||attackingNationMatch;const defendingCondition=!defendingNationFilter||defendingNationMatch;return attackingCondition&&defendingCondition});updateNationSDIBox(attackingNationFilter,defendingNationFilter);currentPage=1;displayNuclearData();setupPagination()}
...
function changePage(newPage){const totalPages=Math.ceil(filteredData.length/itemsPerPage);if(newPage<1||newPage>totalPages)return;currentPage=newPage;displayNuclearData();setupPagination();tableContainer.scrollIntoView({behavior:'smooth',block:'start'})}
